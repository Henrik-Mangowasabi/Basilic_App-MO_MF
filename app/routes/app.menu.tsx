...
export const action = async ({ request }: { request: Request }) => {
    ...
    if (actionType === "set_review_status" || actionType === "clear_review_status") {
        try {
            ...
            return { ok: false, errors: [{ message: "Erreur DB" }] };
        }
    }
    ...
    if (actionType === "update") {
        ...
        } catch (e) { return { ok: false, errors: [{ message: "Erreur update" }] }; }
    }
    return null;
};
...
export default function AppMenu() {
    ...
    const renderCell = (item: MenuItem, key: React.Key) => {
        switch (key) {
            ...
            case "handle": return (<div className="mf-cell mf-cell--key"><span className="mf-text--key text-[#18181B]">{item.handle || "—"}</span></div>);
            case "inCode": {
                const inCode = menuResults.has(item.handle);
                return (<div className="mf-cell mf-cell--center mf-cell--badge"><span className={`mf-badge--code ${inCode ? "mf-badge--found" : ""} ${!inCode ? "text-[#18181B]" : ""}`}>{inCode ? "Oui" : "Non"}</span></div>);
            }
    ...
    return (
        <div className="min-h-screen bg-white animate-in fade-in duration-500">
            <div className="mx-auto px-6 pt-6 pb-32 space-y-6" style={{ maxWidth: "1800px" }}>
                <div className="flex justify-between items-center w-full p-4 bg-default-100 rounded-[16px]"><AppBrand /><BasilicButton variant="flat" className="bg-white border border-[#E4E4E7] text-[#18181B] hover:bg-[#F4F4F5]" isLoading={isScanning} onPress={() => startScan()} icon={isScanning ? null : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/></svg>}>Scan code</BasilicButton></div>
                <div className="flex items-center justify-between w-full flex-wrap gap-4"><NavigationTabs activePath="/app/menu" counts={{ mf: mfCount, mo: moCount, t: totalTemplates, m: mediaCount, menu: menuCount }} /><div style={{ width: "320px" }}><BasilicSearch value={search} onValueChange={setSearch} placeholder="Rechercher un menu…" /></div></div>
                {filtered.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 bg-[#F4F4F5]/50 rounded-[32px] border-2 border-dashed border-[#E4E4E7] animate-in fade-in duration-300"><div className="w-16 h-16 rounded-full bg-white flex items-center justify-center shadow-sm mb-4"><Icons.Menu className="text-[#A1A1AA]" /></div><div className="text-[17px] font-semibold text-[#18181B] mb-2">Aucun menu trouvé</div></div>
                ) : (
                    <div className="mf-section animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        {isTableOpen && (
                            <div className="mf-table__base animate-in fade-in zoom-in-95 duration-300">
                                <Table aria-label="Menus" removeWrapper selectionMode="multiple" selectionBehavior={"checkbox" as any} onRowAction={() => {}} selectedKeys={selectedKeys as any} onSelectionChange={(k: any) => setSelectedKeys(new Set(k === "all" ? filtered.map(f => f.id) : Array.from(k) as string[]))} className="mf-table mf-table--menus" classNames={{ th: "mf-table__header", td: "mf-table__cell", tr: "mf-table__row" }}>
                                    <TableHeader columns={[{ key: "name", label: "NOM DU MENU", className: "mf-col--name" }, { key: "handle", label: "HANDLE", className: "mf-col--key" }, { key: "inCode", label: "DANS LE CODE (TEMPLATES)", className: "mf-col--code" }, { key: "actions", label: "LIEN", className: "mf-col--actions" }, { key: "menu", label: " ", className: "mf-col--menu" }]}>{(c: any) => (<TableColumn key={c.key} align={c.key === "inCode" || c.key === "actions" || c.key === "menu" ? "center" : "start"} className={c.className}>{c.label}</TableColumn>)}</TableHeader>
                                    <TableBody items={filtered} emptyContent="Aucun menu.">{(item: any) => (<TableRow key={item.id} className={reviewStatusMap?.[item.id] === "to_review" ? "mf-table__row--to-review" : reviewStatusMap?.[item.id] === "reviewed" ? "mf-table__row--reviewed" : undefined}>{(key) => <TableCell>{renderCell(item, key)}</TableCell>}</TableRow>)}</TableBody>
                                </Table>
                            </div>
                        )}
                    </div>
            ...
            {selectedKeys.size > 0 && (
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4 duration-300"><div className="flex items-center gap-4 bg-[#18181B] p-2 pl-5 pr-2 rounded-full shadow-2xl ring-1 ring-white/10"><div className="flex items-center gap-3"><span className="text-[14px] font-medium text-white">{selectedKeys.size} sélectionnés</span><button onClick={() => setSelectedKeys(new Set())} className="text-[#A1A1AA] hover:text-white transition-colors"><svg width="20" height="20" viewBox="0 0 20 20" fill="none"><g opacity="0.8"><path d="M10 18.3333C14.6024 18.3333 18.3333 14.6023 18.3333 9.99999C18.3333 5.39762 14.6024 1.66666 10 1.66666C5.39763 1.66666 1.66667 5.39762 1.66667 9.99999C1.66667 14.6023 5.39763 18.3333 10 18.3333Z" fill="#3F3F46"/><path d="M12.5 7.5L7.5 12.5M7.5 7.5L12.5 12.5" stroke="#A1A1AA" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></g></svg></button></div><div className="h-6 w-[1px] bg-[#3F3F46]" /><Button onPress={() => { const fd = new FormData(); fd.append("action", "set_review_status"); fd.append("ids", JSON.stringify(Array.from(selectedKeys))); fd.append("status", "to_review"); submit(fd, { method: "post" }); }} className="bg-[#71717A] text-white font-medium px-4 h-[36px] rounded-full hover:bg-[#52525B] transition-colors gap-2">À review</Button><Button onPress={() => { const fd = new FormData(); fd.append("action", "set_review_status"); fd.append("ids", JSON.stringify(Array.from(selectedKeys))); fd.append("status", "reviewed"); submit(fd, { method: "post" }); }} className="bg-[#3F3F46] text-white font-medium px-4 h-[36px] rounded-full hover:bg-[#27272A] transition-colors gap-2">Review</Button><Button onPress={() => { const fd = new FormData(); fd.append("action", "clear_review_status"); fd.append("ids", JSON.stringify(Array.from(selectedKeys))); submit(fd, { method: "post" }); }} variant="flat" className="text-[#A1A1AA] font-medium px-4 h-[36px] rounded-full hover:bg-white/10 hover:text-white transition-colors">Réinitialiser</Button><div className="h-6 w-[1px] bg-[#3F3F46]" /><Button onPress={() => { setDeleteTarget({ single: null, ids: Array.from(selectedKeys) }); setDeleteModalOpen(true); }} className="bg-[#F43F5E] text-white font-medium px-4 h-[36px] rounded-full hover:bg-[#E11D48] transition-colors gap-2" startContent={<Icons.Delete />}>Supprimer</Button></div></div>
            )}
            <BasilicModal isOpen={editModalOpen} onClose={() => setEditModalOpen(false)} title="Editer le menu" footer={<><Button variant="light" onPress={() => setEditModalOpen(false)} className="grow bg-[#F4F4F5]">Annuler</Button><BasilicButton className="grow" onPress={() => { const fd = new FormData(); fd.append("action", "update"); fd.append("id", editItem?.id!); fd.append("title", editName); fd.append("handle", editHandle); submit(fd, { method: "post" }); }}>Enregistrer</BasilicButton></>}><div className="space-y-4 pt-2"><div><label className="text-[11px] font-bold text-[#71717A] uppercase tracking-wider mb-1.5 block">Nom</label><input value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full h-11 px-4 bg-white border border-[#E4E4E7] rounded-[12px]" /></div><div><label className="text-[11px] font-bold text-[#71717A] uppercase tracking-wider mb-1.5 block">Handle</label><input value={editHandle} onChange={(e) => setEditHandle(e.target.value)} className="w-full h-11 px-4 bg-white border border-[#E4E4E7] rounded-[12px] font-mono" /></div></div></BasilicModal>
            <BasilicModal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} title="Suppression de menus" footer={<><Button variant="light" onPress={() => setDeleteModalOpen(false)} className="grow bg-[#F4F4F5]">Fermer</Button><a href={deleteTarget.single ? getMenuAdminUrl(deleteTarget.single) : `https://admin.shopify.com/store/${storeSlug}/content/menus`} target="_blank" rel="noopener noreferrer" className="inline-flex grow items-center justify-center gap-2 rounded-xl bg-[#4BB961] px-4 py-2.5 text-[14px] font-semibold text-white hover:opacity-90 transition-opacity">Ouvrir l&apos;admin Shopify</a></>}><p className="py-2 text-sm text-[#71717A]">La suppression de menus n&apos;est pas disponible via l&apos;API. Supprimez les menus depuis l&apos;admin Shopify.</p></BasilicModal>
